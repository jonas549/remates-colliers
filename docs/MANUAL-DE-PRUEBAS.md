...
| Un correo no llega | El servidor lo aceptó pero se filtró, o el remitente no está autorizado | `/admin/configuracion/correos`: si figura «enviada», el servidor de salida lo aceptó (revisa spam y los registros del proveedor); si figura «fallida», ahí está el error completo. Si el remitente no es el usuario SMTP, la pantalla de Correo lo avisa. |
